...
// 🚀 ADD THIS NEW STATE CONTROLLER TO HANDLE DELETIONS MID-SESSION:
    ...
    function renderModalAttachments() {
       ...
       async function saveItem() {
        ...
        // Keep local copy of attachments before saving
        const attachmentsToSave = [...currentItemAttachments];
        ...
        try {
            ...
            // ✨ THE WORKFLOW INTERCEPTOR FOR NEW ITEMS
            // If we just created a brand new item AND have locally staged files, link them now
            if (!editingItemId && attachmentsToSave.length > 0) {
                // Try to extract the fresh item ID generated by your backend Lambda
                const newLiveItemId = savedItemResult.itemId || savedItemResult.Item?.itemId || `ITEM${Date.now()}`;
                
                console.log(`🔗 Linking ${attachmentsToSave.length} staged attachments to fresh item row ID:`, newLiveItemId);

                const cleanContainerId = String(activeShortContainerId).replace("CONTAINER#", "").trim();

                // Loop through and link each staged file using your working attachment endpoint
                for (const att of attachmentsToSave) {
                    try {
                        const dbPayload = {
                            pk: `CONTAINER#${cleanContainerId.toUpperCase()}`,
                            sk: `ITEM#${newLiveItemId}`,
                            filename: att.filename || att.label,
                        ...
                        console.error("⚠️ Background file linkage failure:", innerErr);
                    }
                }
            }

            // Force data synchronization from cloud completely
            await loadItems();
            
            // Wipe modal and close interface components safely
            closeItemModal();
            
            alert("Item and all attachments processed successfully!");
